...
export async function encryptFiles(
    ...
    if (!password || password.trim() === "") { // no pwd, only for zip
        return zippedData.buffer.slice(zippedData.byteOffset, zippedData.byteOffset + zippedData.byteLength);
    }
    ...
    let offset = 0;
    result.set(MAGIC_ENCRYPTED, offset); offset += MAGIC_ENCRYPTED.length;
    result.set(salt, 0);
...
export async function decryptFiles(
    ...
    if (isRawZip) {
        ...
        const salt = fullData.subarray(offset, offset + 16); offset += 16;
        const iv = fullData.subarray(offset, offset + 12);   offset += 12;
